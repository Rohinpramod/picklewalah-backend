...
exports.addQuantity = async (req, res) => {
  try {
    ...
    if (action === "increment") {
      item.quantity += 1;
      item.totalItemPrice += item.itemId.price;
      cart.totalPrice += item.itemId.price;
...
exports.getCart = async (req, res) => {
  try {
    ...
    const cart = await  Cart.findOne({ userId ,cartStatus: { $ne: "ordered" } }).populate("items.itemId", "name image price", null, { lean: true });
    if (!cart) {
